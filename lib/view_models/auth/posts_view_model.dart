...
class PostsViewModel extends ChangeNotifier {
  ...
  pickImage({bool camera = false, BuildContext context}) async {
    ...
    try {
      ...
      loading = false;
      notifyListeners();
      showInSnackBar('Цуцлагдсан', context);
    }
  }
  ...
  getLocation() async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      location = " ${placemarks[0].street}, ${placemarks[0].subLocality}";
  ...
  uploadPosts(BuildContext context) async {
    try {
      ...
      loading = false;
      resetPost();
      showInSnackBar('Амжилттай бүртгэгдлээ!', context);
      notifyListeners();
    }
  }
  ...
  uploadProfilePicture(BuildContext context) async {
    if (mediaUrl == null) {
      showInSnackBar('Зураг сонгоно уу', context);
    } else {
      try {
        loading = true;
        notifyListeners();
